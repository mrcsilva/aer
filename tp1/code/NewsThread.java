        ...
        if(flag == 1) {
            ...
            if(self.equals(viz)) {
                try {
                    ...
                    if(tabela.containsKey(dip) && tabela.get(dip).getSaltos() != 0){
                        try{
                            so.joinGroup(group);
                            String temp = "NEWS_FOR " + source + " " + dest + " " + tabela.get(dip).getIpVizinho().getHostAddress() + " " + dataTcp;
                            buf = temp.getBytes();
                            ...
                            System.out.println("NT Send e N!=0: ");
                            e.printStackTrace();
                        }
                    }
            ...
            if(self.equals(viz)) {
                ...
                if(tabela.containsKey(dip) && tabela.get(dip).getSaltos() != 0){
                    try{
                        so.joinGroup(group);
                        String temp = "GET_NEWS_FROM " + source + " " + dest + " " + tabela.get(dip).getIpVizinho().getHostAddress();
                        buf = temp.getBytes();
                        p = new DatagramPacket(buf, buf.length, group, 9999);
                        so.send(p);
                        so.leaveGroup(group);
